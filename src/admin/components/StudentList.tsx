...
import { Search, Eye, ArrowLeft, GraduationCap } from "lucide-react";
import { fetchAllStudents, type StudentRecord } from "@/admin/services/student-admin.service";
...
export function StudentList({ onSelectStudent, onRefresh, onBack }: StudentListProps) {
  ...
  const getStatusBadge = (status: string) => {
    const styles = {
      ...
      inactive: "bg-gray-50 text-gray-700 border-gray-200 shadow-sm",
      pending: "bg-amber-50 text-amber-700 border-amber-200 shadow-sm",
    };

    return (
      <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold border ${styles[status as keyof typeof styles] || styles.pending}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="text-center py-32">
            ...
            <p className="mt-6 text-sm font-medium text-gray-600">Loading students...</p>
          </div>
                  ...
                  <tr key={student.id} className="hover:bg-gray-50 transition-colors group">
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center font-bold text-white text-sm shadow-md">
                    ...
                    <td className="px-6 py-5">
                      <span className="text-sm text-gray-900">{student.applicant_type}</span>
                    </td>
                    <td className="px-6 py-5">
