...
import { login, getRedirectPath, isAnyAdmin } from '@/services/auth.service';
...
export function UnifiedLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        if (isAnyAdmin(result.user.role) && checkIsMobile()) {
  ...
  return (
    <div className="w-screen h-screen overflow-hidden bg-gray-100">
      <div className="relative h-screen w-full max-w-[430px] mx-auto flex flex-col bg-white">
